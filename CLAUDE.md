...
| Titel en tags uit tekst | Suggestie (`suggestie.py`, pure functies): titel = alleen de afzender, bij twijfel leeg; heuristiek op prioriteit: bekende archieftitel in de tekst → naam achter "t.n.v." → eerste kolomcel met rechtsvorm (`B.V.` e.d., hoofdlettergevoelig) of instantiewoord (Gemeente …, Belastingdienst …) → bij korte tekst (bon, < 25 regels) de eerste bruikbare regel. Tags = documenttypewoorden als kopregel van een cel (factuur, offerte, polis, …). Cellen = splitsing op 2+ spaties. De inbox gebruikt de suggestie (anders bestandsnaam); het uploadformulier vanaf 15b. `ingest.py` in twee fasen: `lees_vooraf` → `maak_document_uit_voorbereid`. Details: `werk/15a-titel-en-tagsuggestie.md`. |
...
| Web | FastAPI + Jinja2, geen JS-framework, geen build-stap. Vanilla JS alleen voor upload-voortgang en status-polling. Alle links via de Jinja-global `url_for` (pad inclusief Ingress `root_path`). |
